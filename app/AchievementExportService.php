    ...
    public function filterOptions(int $clubId): array
    {
        ...
        $sessions = $this->pdo->prepare(
            'SELECT id, season_id, name, day_of_week, start_time
             ...
             WHERE club_id = :club_id AND active = 1 AND deleted_at IS NULL
        ...
        $groups = $this->pdo->prepare(
            'SELECT pg.id, pg.program_session_id, pg.name, ps.season_id, ps.name AS session_name
             ...
             WHERE pg.active = 1 AND pg.deleted_at IS NULL
    ...
    private function validateRegistrationFilters(
            ...
            $statement = $this->pdo->prepare(
                'SELECT COUNT(*) FROM program_session
                 WHERE id = :session_id AND season_id = :season_id
                   AND club_id = :club_id AND active = 1 AND deleted_at IS NULL'
            );
            $statement->execute([
        ...
        if ($groupId !== null) {
            $sql = 'SELECT COUNT(*)
                    ...
                    WHERE pg.id = :group_id AND pg.active = 1 AND pg.deleted_at IS NULL
                      AND ps.season_id = :season_id AND ps.club_id = :club_id
                      AND ps.active = 1 AND ps.deleted_at IS NULL';
    ...
    private function skaters(
        ...
        if ($sessionId !== null) {
            ...
            $groupJoins = '
             LEFT JOIN skater_enrollment export_enrollment
               ...
               AND export_enrollment.active = 1
        ...
        $statement = $this->pdo->prepare(
            'SELECT DISTINCT
             ...
             ' . $groupJoins . '
             WHERE s.club_id = :club_id AND s.active = 1 AND s.deleted_at IS NULL
               AND EXISTS (
                    SELECT 1
